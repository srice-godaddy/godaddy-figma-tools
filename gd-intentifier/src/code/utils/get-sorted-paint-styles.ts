...
export default function getSortedPaintStyles(
    needleColor: RGB,
    paintStyles: PaintStyle[]
): PaintStyle[] {
    const rgb1 = figmaRgbToRgb(needleColor);

    const weighedDistance = [...paintStyles]
        .map((paintStyle) => {
        ...
        .sort(function compareDistances(
            paintStyleDistance1,
            paintStyleDistance2
        ) {
    ...
    return weighedDistance.map(
        (paintStyleWithDistance) => paintStyleWithDistance.paintStyle
    );
}
