...
export default class IntentRecommendationService {
    ...
    constructor(figmaInstance: PluginAPI, intentStyles: IntentStylesService) {
        ...
        const {
            textStyles,
            paintStyles,
        } = this.intentStyles.getValidIntentStyles();

        this.textStyles = textStyles;
        this.paintStyles = paintStyles;
        this.styleIds = [...textStyles, ...paintStyles].map(
            (style) => style.id
        );
    ...
    getRecommendedSelectionStyles(
        selection: ReadonlyArray<SceneNode>
    ): { [id: string]: Array<TextStyle | PaintStyle> } {
    ...
    getRecommendedPaintStyles(
        node: SupportedPaintStylesType
    ): {
        fillStyles: Array<PaintStyle>;
    } {
        ...
        const sortedPaintStyles = getSortedPaintStyles(
            node.fills?.[0]?.color,
    ...
    getRecommendedTextStyles(
        ...
        return {
            ...
            ...(!hasValidFillStyle && {
                textFillStyles: getSortedPaintStyles(
                    node.fills?.[0]?.color,
    ...
    getNodeStyleFixes(node: SceneNode) {
        switch (node.type) {
            ...
            case 'INSTANCE':
                ...
                const shouldHaveStrokeStyle = !!relatedPaintStyles?.strokeStyle
                    ?.id;

                const hasFillStyle = !isEmpty(node.fills) || !!node.fillStyleId;
                const hasStrokeStyle =
                    !isEmpty(node.strokes) || !!node.strokeStyleId;

                if (
                    shouldHaveFillStyle !== hasFillStyle ||
                    shouldHaveStrokeStyle !== hasStrokeStyle ||
                    (node.fillStyleId || null) !==
                        (relatedPaintStyles?.fillStyle?.id || null) ||
                    (node.strokeStyleId || null) !==
                        (relatedPaintStyles?.strokeStyle?.id || null)
                ) {
    ...
    getMainPaintStyle(node): [string, PaintStyle] {
        ...
        const mainPaintStyleKey = prioritizedPaintStyles.find(
            (paintStyleIdKey) => this.isValidStyleId(node[paintStyleIdKey])
        );
        ...
        const paintStyle: PaintStyle = this.figmaInstance.getStyleById(
            node[mainPaintStyleKey]
        ) as PaintStyle;
