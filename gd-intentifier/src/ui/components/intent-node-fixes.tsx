...
export default function IntentNodeFixes({ items }) {
    ...
    return (
        <div className="ui-intent-fixes">
            <h2 className="ui-intent-fixes__heading">
                Existing styles don't use complete intent set. Click on preview
                to fix.
            </h2>
            ...
            {items.map((item) => (
                <StylePreview
                    ...
                    onStylePreviewClick={(styleIds) =>
                        handleStylePreviewClick(item.nodeId, styleIds)
                    }
                    onStylePreviewMouseEnter={(styleIds) =>
                        handleStylePreviewMouseEnter(item.nodeId, styleIds)
                    }
                    onStylePreviewMouseLeave={() =>
                        handleStylePreviewMouseLeave(item.nodeId)
                    }
                />
            ))}
        </div>
